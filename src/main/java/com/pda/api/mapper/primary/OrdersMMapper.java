...
public interface OrdersMMapper extends BaseMapper<OrdersM> {
    ...
    @Select("select * from orders_m where patient_id = #{patientId} and start_date_time <= #{endTime} and repeat_indicator = 1 and stop_date_time is null and administration in ('续静滴','静脉输液') order by order_no")
    List<OrdersM> listByPatientId(@Param("patientId") String patientId, @Param("endTime") Date endTime);

    @Select("select * from orders_m where patient_id = #{patientId} and repeat_indicator = 0 and start_date_time >= #{startDateOfDay} and start_date_time <= #{endDateOfDay} and administration in ('续静滴','静脉输液') order by order_no")
    ...
    @Select("select * from orders_m where patient_id = #{patientId} and repeat_indicator = 0 and start_date_time >= #{startDateOfDay} and start_date_time <= #{endDateOfDay} and administration = '口服' order by order_no")
    ...
    List<OrdersM> listShortOrderByPatientId(@Param("patientId") String patientId,@Param("startDateOfDay") Date startDateOfDay,@Param("endDateOfDay") Date endDateOfDay,@Param("type") String type);

    List<OrdersM> listLongOrderByPatientId(@Param("patientId") String patientId,@Param("queryTime") Date queryTime,@Param("type") String type);
}
